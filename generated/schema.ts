...
export class ItemDelisted extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type ItemDelisted must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ItemDelisted", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): ItemDelisted | null {
    return changetype<ItemDelisted | null>(
      store.get("ItemDelisted", id.toHexString())
    );
  }
